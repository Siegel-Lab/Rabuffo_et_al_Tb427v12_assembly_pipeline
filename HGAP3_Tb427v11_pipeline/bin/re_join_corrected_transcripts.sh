...
cat <(grep '^##' ${GFF_FOLDER}/${COMPANION_GFF} | grep -v '^###') <(cat <(grep -v -P '\t(pseudo)?gene\t|\tmRNA\t|\tpseudogenic_transcript\t' ${GFF_FOLDER}/${GFF_IN_ONE}) ${GFF_FOLDER}/${GFF_IN_TWO} | sort -t$'\t' -k 1,1 -k4n,4n) > ${GFF_FOLDER}/${OUT_GFF}
