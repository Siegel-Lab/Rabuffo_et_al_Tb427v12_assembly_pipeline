...
#my $inc;
my $VSG_ID;
my $Aln_length_times_match_perc;
#my $Match_perc;
#Name=Similar to Tb427VSG-1080;Note=From 50533 to 51990, 100.000 percent identity to VSG Tb427VSG-1080. The alignment length is 1458 nt, while the query VSG gene sequence is 1458 nt long. This is the most similar gene to Tb427VSG-1080 (Tb427VSG-1080 : GB Accession KC612589 : 1458nt : N-terminus partial : contig5313.) in the genome.
...
#			$VSG_Name =~ s/;$//;
			# if the current VSG alignment is smaller than the previous one, keep the VSG_ID of the previous one for the naming 
			if (($VSG_End - $VSG_Start) < ($last->[4] - $last->[3])) {
#			$VSG_Description = $VSG_Name =~ s/Similar to //r;
			$VSG_Attributes = $last->[8]
			# don't do anything
			}
