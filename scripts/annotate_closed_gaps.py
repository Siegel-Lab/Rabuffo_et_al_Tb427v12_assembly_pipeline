...
def annotate_closed_gaps(old_genome, new_genome, old_gaps, rev_complemented_contigs, overhang=1000, gap_size=1000):
    ...
    with open(rev_complemented_contigs, "w") as out_file:
                ...
                for jdx, old_scaffold in enumerate(old_scaffolds):
                    ...
                    for idx, new_scaffold in enumerate(new_scaffolds):
                        if cropped_scaffold in new_scaffold:
                            curr_assignment.append((idx, True))
                        elif old_scaffold_rev_comp in new_scaffold:
                            curr_assignment.append((idx, False))
                    if not len(curr_assignment) == 1:
                        ...
                        if not curr_assignment[0][1]:
                            print("WARNING: reverse complement assignment:", contig_name, "scaffold", jdx, 
                                  curr_assignment, file=sys.stderr)
                        scaffold_assignment.append(curr_assignment[0][0])
